...
class LLMWildcardManager:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                # When ON, the edited design brief is reused as-is and the LLM
                # brief step is skipped. When OFF (default), the brief is
                # regenerated by the LLM on every queue and any user edits to
                # the brief are ignored — lock to preserve hand-edits.
                "lock_brief": ("BOOLEAN", {
                    "default": False,
                    "tooltip": (
                        "When ON, reuse the design brief on the node and skip "
                        "the LLM brief call. When OFF, regenerate the brief "
                        "from the LLM every queue (edits are overwritten)."
                    ),
                }),
            },
            "optional": {
    ...
    def manage(self, server, example_prompt, lock_template, seed, direction,
               ...
               design_brief="", lock_brief=False, structure=None):
        ...
        # When the brief lock is OFF, treat the persisted brief as throwaway —
        # the LLM regenerates it from the idea every queue.
        if not lock_brief:
            brief_text = ""
